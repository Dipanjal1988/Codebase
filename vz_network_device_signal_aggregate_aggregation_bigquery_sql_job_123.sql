...
DECLARE v_job_id INT64 DEFAULT 123;
CREATE TEMP TABLE stg_network_device_signal_123 AS
SELECT
  s.site_id,
       s.cell_id,
       s.node_id,
...
SELECT * FROM stg_network_device_signal_123;
...
VALUES (123, 'vz_aggregation_network_device_signal', '`vz-agentic-198889.ftr_raw.network_device_signal`', '`vz-agentic-198889.vz_curated.network_device_signal_aggregation`', 'network', 'aggregation', 'bigquery_sql', CURRENT_TIMESTAMP());
