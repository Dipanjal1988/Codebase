/*
FILE_NAME_CONTEXT: Verizon network.fiber_node aggregation lineage job
JOB_ID: 283
JOB_TYPE: aggregation
DIALECT: bigquery_sql
SOURCE_OBJECT: ftr_raw.network_fiber_node
TARGET_OBJECT: vz_curated.network_fiber_node_aggregation
*/
-- DIALECT DETAIL: BIGQUERY SQL
DECLARE v_job_id INT64 DEFAULT 283;
CREATE TEMP TABLE stg_network_fiber_node_283 AS
SELECT
  s.site_id,
       s.cell_id,
       s.node_id,
...
FROM `vz-agentic-198889.ftr_raw.network_fiber_node` s
WHERE COALESCE(s.record_status, 'ACTIVE') = 'ACTIVE';

CREATE OR REPLACE TABLE `vz-agentic-198889.vz_curated.network_fiber_node_aggregation` AS
SELECT * FROM stg_network_fiber_node_283;

CREATE OR REPLACE TABLE `vz-agentic-198889.vz_mart.network_fiber_node_aggregation_daily_kpi` AS
SELECT source_system, DATE(lineage_load_ts) AS kpi_dt, COUNT(*) AS record_count
FROM `vz-agentic-198889.vz_curated.network_fiber_node_aggregation`
...
VALUES (283, 'vz_aggregation_network_fiber_node', '`vz-agentic-198889.ftr_raw.network_fiber_node`', '`vz-agentic-198889.vz_curated.network_fiber_node_aggregation`', 'network', 'aggregation', 'bigquery_sql', CURRENT_TIMESTAMP());
