/*
FILE_NAME_CONTEXT: Verizon network.network_event aggregation lineage job
JOB_ID: 263
JOB_TYPE: aggregation
DIALECT: bigquery_sql
SOURCE_OBJECT: ftr_raw.network_network_event
TARGET_OBJECT: vz_curated.network_network_event_aggregation
*/
-- DIALECT DETAIL: BIGQUERY SQL
DECLARE v_job_id INT64 DEFAULT 263;
CREATE TEMP TABLE stg_network_network_event_263 AS
SELECT
  s.site_id,
       s.cell_id,
       s.node_id,
...
FROM `vz-agentic-198889.ftr_raw.network_network_event` s
WHERE COALESCE(s.record_status, 'ACTIVE') = 'ACTIVE';

CREATE OR REPLACE TABLE `vz-agentic-198889.vz_curated.network_network_event_aggregation` AS
SELECT * FROM stg_network_network_event_263;

CREATE OR REPLACE TABLE `vz-agentic-198889.vz_mart.network_network_event_aggregation_daily_kpi` AS
SELECT source_system, DATE(lineage_load_ts) AS kpi_dt, COUNT(*) AS record_count
FROM `vz-agentic-198889.vz_curated.network_network_event_aggregation`
...
VALUES (263, 'vz_aggregation_network_network_event', '`vz-agentic-198889.ftr_raw.network_network_event`', '`vz-agentic-198889.vz_curated.network_network_event_aggregation`', 'network', 'aggregation', 'bigquery_sql', CURRENT_TIMESTAMP());
