/*
FILE_NAME_CONTEXT: Verizon usage.cdr_sms validation lineage job
JOB_ID: 275
JOB_TYPE: validation
DIALECT: spark_sql
SOURCE_OBJECT: ftr_raw.usage_cdr_sms
TARGET_OBJECT: vz_curated.usage_cdr_sms_validation
*/
-- DIALECT DETAIL: SPARK SQL
CREATE OR REPLACE TEMP VIEW stg_usage_cdr_sms_275 AS
SELECT
  s.usage_id,
       s.customer_id,
       s.msisdn,
...
FROM ftr_raw.usage_cdr_sms s
WHERE COALESCE(s.record_status, 'ACTIVE') = 'ACTIVE';

MERGE INTO vz_curated.usage_cdr_sms_validation t
USING stg_usage_cdr_sms_275 s
...
CREATE OR REPLACE TABLE vz_mart.usage_cdr_sms_validation_summary AS
SELECT source_system, COUNT(*) AS record_count, MAX(lineage_load_ts) AS latest_load_ts
FROM vz_curated.usage_cdr_sms_validation
GROUP BY source_system;

INSERT INTO audit_control.table_lineage
SELECT 275, 'vz_validation_usage_cdr_sms', 'ftr_raw.usage_cdr_sms', 'vz_curated.usage_cdr_sms_validation', 'usage', 'validation', 'spark_sql', CURRENT_TIMESTAMP;
