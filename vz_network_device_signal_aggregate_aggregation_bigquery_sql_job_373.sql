/*
FILE_NAME_CONTEXT: Verizon network.device_signal aggregation lineage job
JOB_ID: 373
JOB_TYPE: aggregation
DIALECT: bigquery_sql
SOURCE_OBJECT: ftr_raw.network_device_signal
TARGET_OBJECT: vz_curated.network_device_signal_aggregation
*/
-- DIALECT DETAIL: BIGQUERY SQL
DECLARE v_job_id INT64 DEFAULT 373;
CREATE TEMP TABLE stg_network_device_signal_373 AS
SELECT
  s.site_id,
       s.cell_id,
       s.node_id,
...
FROM `vz-agentic-198889.ftr_raw.network_device_signal` s
WHERE COALESCE(s.record_status, 'ACTIVE') = 'ACTIVE';

CREATE OR REPLACE TABLE `vz-agentic-198889.vz_curated.network_device_signal_aggregation` AS
SELECT * FROM stg_network_device_signal_373;

CREATE OR REPLACE TABLE `vz-agentic-198889.vz_mart.network_device_signal_aggregation_daily_kpi` AS
SELECT source_system, DATE(lineage_load_ts) AS kpi_dt, COUNT(*) AS record_count
FROM `vz-agentic-198889.vz_curated.network_device_signal_aggregation`
...
VALUES (373, 'vz_aggregation_network_device_signal', '`vz-agentic-198889.ftr_raw.network_device_signal`', '`vz-agentic-198889.vz_curated.network_device_signal_aggregation`', 'network', 'aggregation', 'bigquery_sql', CURRENT_TIMESTAMP());
