/*
FILE_NAME_CONTEXT: Verizon usage.roaming_usage validation lineage job
JOB_ID: 345
JOB_TYPE: validation
DIALECT: spark_sql
SOURCE_OBJECT: ftr_raw.usage_roaming_usage
TARGET_OBJECT: vz_curated.usage_roaming_usage_validation
*/
-- DIALECT DETAIL: SPARK SQL
CREATE OR REPLACE TEMP VIEW stg_usage_roaming_usage_345 AS
SELECT
  s.usage_id,
       s.customer_id,
       s.msisdn,
...
FROM ftr_raw.usage_roaming_usage s
WHERE COALESCE(s.record_status, 'ACTIVE') = 'ACTIVE';

MERGE INTO vz_curated.usage_roaming_usage_validation t
USING stg_usage_roaming_usage_345 s
...
CREATE OR REPLACE TABLE vz_mart.usage_roaming_usage_validation_summary AS
SELECT source_system, COUNT(*) AS record_count, MAX(lineage_load_ts) AS latest_load_ts
FROM vz_curated.usage_roaming_usage_validation
GROUP BY source_system;

INSERT INTO audit_control.table_lineage
SELECT 345, 'vz_validation_usage_roaming_usage', 'ftr_raw.usage_roaming_usage', 'vz_curated.usage_roaming_usage_validation', 'usage', 'validation', 'spark_sql', CURRENT_TIMESTAMP;
