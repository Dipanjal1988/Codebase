/*
FILE_NAME_CONTEXT: Verizon billing.bill_cycle curation lineage job
JOB_ID: 142
JOB_TYPE: curation
DIALECT: oracle_plsql
SOURCE_OBJECT: ftr_raw.billing_bill_cycle
TARGET_OBJECT: vz_curated.billing_bill_cycle_curation
*/
CREATE OR REPLACE PROCEDURE vz_curation_billing_bill_cycle_p AS
  v_job_id NUMBER := 142;
BEGIN
  INSERT INTO audit_control.job_run_log(job_id, job_name, domain_name, run_status, start_ts)
  VALUES(v_job_id, 'vz_curation_billing_bill_cycle', 'billing', 'STARTED', SYSTIMESTAMP);

  DELETE FROM vz_curated.billing_bill_cycle_curation t
   WHERE EXISTS (SELECT 1 FROM ftr_raw.billing_bill_cycle s WHERE s.billing_account_id = t.billing_account_id);

  INSERT INTO vz_curated.billing_bill_cycle_curation (billing_account_id, invoice_id, cycle_code, bill_period_start_dt, bill_period_end_dt, charge_amt, tax_amt, discount_amt, payment_amt, balance_amt, currency_code, payment_status, autopay_flag, late_fee_amt, adjustment_reason, batch_id, source_system, load_ts, effective_start_ts, effective_end_ts, record_status, hash_key, created_by, updated_by, lineage_load_ts, lineage_job_id)
  ...
  FROM ftr_raw.billing_bill_cycle s
  ...
  VALUES(v_job_id, 'vz_curation_billing_bill_cycle', 'ftr_raw.billing_bill_cycle', 'vz_curated.billing_bill_cycle_curation', 'billing', 'curation', 'oracle_plsql', SYSTIMESTAMP);
