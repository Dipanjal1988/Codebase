...
DECLARE v_job_id INT64 DEFAULT 403;
CREATE TEMP TABLE stg_network_cell_site_403 AS
SELECT
  s.site_id,
       s.cell_id,
       s.node_id,
...
SELECT * FROM stg_network_cell_site_403;
...
VALUES (403, 'vz_aggregation_network_cell_site', '`vz-agentic-198889.ftr_raw.network_cell_site`', '`vz-agentic-198889.vz_curated.network_cell_site_aggregation`', 'network', 'aggregation', 'bigquery_sql', CURRENT_TIMESTAMP());
