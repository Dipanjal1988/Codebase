/*
FILE_NAME_CONTEXT: Verizon orders.fulfillment_event scheduling lineage job
JOB_ID: 324
JOB_TYPE: scheduling
DIALECT: snowflake_sql
SOURCE_OBJECT: ftr_raw.orders_fulfillment_event
TARGET_OBJECT: vz_curated.orders_fulfillment_event_scheduling
*/
-- DIALECT DETAIL: SNOWFLAKE SQL
CREATE OR REPLACE TEMP VIEW stg_orders_fulfillment_event_324 AS
SELECT
  s.order_id,
       s.order_line_id,
       s.customer_id,
...
FROM ftr_raw.orders_fulfillment_event s
WHERE COALESCE(s.record_status, 'ACTIVE') = 'ACTIVE';

MERGE INTO vz_curated.orders_fulfillment_event_scheduling t
USING stg_orders_fulfillment_event_324 s
ON t.order_id = s.order_id
WHEN MATCHED THEN UPDATE SET lineage_load_ts = s.lineage_load_ts
WHEN NOT MATCHED THEN INSERT (order_id, order_line_id, customer_id, product_id, order_status, order_channel, submitted_ts, completed_ts, activation_ts, fulfillment_status, ship_tracking_id, sales_rep_id, store_id, install_type, batch_id, source_system, load_ts, effective_start_ts, effective_end_ts, record_status, hash_key, created_by, updated_by, fulfillment_event_attr_1, lineage_load_ts, lineage_hash)
VALUES (s.order_id, s.order_line_id, s.customer_id, s.product_id, s.order_status, s.order_channel, s.submitted_ts, s.completed_ts, s.activation_ts, s.fulfillment_status, s.ship_tracking_id, s.sales_rep_id, s.store_id, s.install_type, s.batch_id, s.source_system, s.load_ts, s.effective_start_ts, s.effective_end_ts, s.record_status, s.hash_key, s.created_by, s.updated_by, s.fulfillment_event_attr_1, s.lineage_load_ts, s.lineage_hash);

CREATE OR REPLACE TABLE vz_mart.orders_fulfillment_event_scheduling_summary AS
SELECT source_system, COUNT(*) AS record_count, MAX(lineage_load_ts) AS latest_load_ts
FROM vz_curated.orders_fulfillment_event_scheduling
GROUP BY source_system;

INSERT INTO audit_control.table_lineage
SELECT 324, 'vz_scheduling_orders_fulfillment_event', 'ftr_raw.orders_fulfillment_event', 'vz_curated.orders_fulfillment_event_scheduling', 'orders', 'scheduling', 'snowflake_sql', CURRENT_TIMESTAMP;
